...
function validateConfirm(inp){
  ...
  document.getElementById('rPwd2Err').style.display=inp.value&&!ok?'block':'none';
...
function checkStrength(inp){
  ...
  bar.style.width=d.w;bar.style.background=d.c;
...
function goStep(n){
  if(n>curStep){
    if(curStep===1){
      ...
      if(!fn||!ln){alert('Veuillez renseigner votre prénom et nom.');return}
      if(!/^[^@]+@[^@]+\.[^@]+$/.test(em)){alert('Adresse e-mail invalide.');return}
      if(pw.length<8){alert('Mot de passe trop court (8 caractères min.).');return}
...
function selectRole(element, codeRole) {
    ...
    const labels = { part: 'Particulier', pro: 'Professionnel', sal: 'Salarié ReNova', adm: 'Administrateur' };
...
async function submitRegister() {
    ...
    let infosAEnvoyer = {
        ...
        role: roleChoisi,
        type_statut: (roleChoisi === "Utilisateur") ? "Validé" : "En attente" 
    };
