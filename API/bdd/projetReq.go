...
func CreateProjet(p models.Projet) error {
	_, err := Db.Exec(
		...
		p.IdUser, p.Titre, p.Description, p.AvantDesc, p.Photo, p.Statut, p.Co2Evite,
	)
	return err
}
...
func GetProjetsByUser(idUser int) ([]models.Projet, error) {
	rows, err := Db.Query(
		`SELECT id_projet, id_user, titre, desc_etapes, url_photo_avant, url_photo_apres, COALESCE(statut, 'en_cours'), COALESCE(co2_evite, 0) FROM projet_pro WHERE id_user = ?`,
		idUser,
	)
	if err != nil {
		return nil, err
	...
	for rows.Next() {
		var p models.Projet
		if err := rows.Scan(&p.Id, &p.IdUser, &p.Titre, &p.Description, &p.AvantDesc, &p.Photo, &p.Statut, &p.Co2Evite); err != nil {
			return nil, err
		}
...
func UpdateProjet(p models.Projet) error {
	_, err := Db.Exec(
		...
		p.Titre, p.Description, p.AvantDesc, p.Photo, p.Statut, p.Co2Evite, p.Id,
	)
	return err
}
